...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super initWithCoder:aDecoder]) {
        ...
        _nameLabel.textColor = [UIColor colorWithRed:34.0/255.0
                                               green:97.0/255.0
                                                blue:221.0/255.0
                                               alpha:1];
        ...
        _balloonInsetsLeft = UIEdgeInsetsMake(BALLOON_INSET_TOP,
                                              BALLOON_INSET_RIGHT,
                                              BALLOON_INSET_BOTTOM,
                                              BALLOON_INSET_LEFT);
        
        _balloonInsetsRight = UIEdgeInsetsMake(BALLOON_INSET_TOP,
                                               BALLOON_INSET_LEFT,
                                               BALLOON_INSET_BOTTOM,
                                               BALLOON_INSET_RIGHT);
...
- (void)setTranscript:(Transcript *)transcript
{
    ...
    _messageLabel.frame = CGRectMake(xOffsetLabel,
                                     yOffset + 5,
                                     labelSize.width,
                                     labelSize.height);
    
    _balloonView.frame = CGRectMake(xOffsetBalloon,
                                    yOffset,
                                    balloonSize.width,
                                    balloonSize.height);
    
    _nameLabel.frame = CGRectMake(xOffsetLabel - 2,
                                  1,
                                  nameSize.width,
                                  nameSize.height);
}
...
{
    CGFloat labelHeight = [MessageView balloonSizeForLabelSize:[MessageView labelSizeForString:transcript.message
                                                                                      fontSize:MESSAGE_FONT_SIZE]].height;
    if (TRANSCRIPT_DIRECTION_SEND != transcript.direction) {
